...
export class ProviderService {
  ...
  async getProviderBookings(providerId: string) {
    ...
    const [{ data: customers }, { data: services }] = await Promise.all([
      ...
      serviceIds.length ? this.catalogDb.from('provider_services').select('id,title').in('id', serviceIds) : Promise.resolve({ data: [] }),
    ]);
  ...
  async getProviderBookingById(bookingId: string) {
    ...
    const [{ data: customer }, { data: service }] = await Promise.all([
      ...
      this.catalogDb.from('provider_services').select('id,title,description,price').eq('id', data.service_id).maybeSingle(),
    ]);
