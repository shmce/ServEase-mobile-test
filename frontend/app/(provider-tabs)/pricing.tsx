...
export default function PricingScreen() {
  ...
  const loadDbCategories = useCallback(async () => {
    ...
    if (error) {
      console.warn('[PricingScreen] loadDbCategories failed:', error.message);
      return;
    }
  ...
  const loadServices = useCallback(async () => {
    ...
    if (error) {
      console.warn('[PricingScreen] loadServices failed:', error.message);
      return;
    }
  ...
  // Resolve a subcategory name to a DB category row id, creating it if needed
  const resolveCategoryId = useCallback(async (subcategoryName: string): Promise<string> => {
    const slug = slugify(subcategoryName);

    // Check cache first
    if (dbCategoryMap[slug]) return dbCategoryMap[slug].id;

    // Try to find in DB
    const { data: existing } = await providerCatalogDb
      .from('service_categories')
      .select('id,name,slug')
      .eq('slug', slug)
      .single();
    ...
    if (existing) {
      setDbCategoryMap((prev) => ({ ...prev, [slug]: existing as CategoryRow }));
      return (existing as CategoryRow).id;
    }

    // Create it
    const parentGroup = findParentCategoryForSubcategory(subcategoryName);
    const { data: inserted, error } = await providerCatalogDb
      .from('service_categories')
      .insert({
    ...
    if (error) {
      throw new Error(`Could not create category "${subcategoryName}": ${error.message}`);
    }

    const row = inserted as CategoryRow;
    setDbCategoryMap((prev) => ({ ...prev, [slug]: row }));
    return row.id;
  }, [dbCategoryMap]);
  ...
  const ensureProviderIdentityRows = useCallback(async () => {
    if (!user?.id) return;
    const meta = (user.user_metadata ?? {}) as Record<string, any>;
    const fullName = String(meta.full_name || meta.name || '').trim() || `Provider ${user.id.slice(0, 8)}`;
    ...
    await providerCatalogDb.from('provider_profiles').upsert({
      user_id: user.id,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Retry after ensuring identity rows exist
