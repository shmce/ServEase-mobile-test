...
import { View, Text, StyleSheet, TouchableOpacity, SafeAreaView, ScrollView, StatusBar, TextInput, Image, Dimensions, KeyboardAvoidingView, Platform, ActivityIndicator, Alert } from 'react-native';
...
export default function ProviderEditProfileScreen() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      if (data && !error) {
        setBusinessName(data.full_name || ""); // Using full_name as business name for now 
        // In a real app, business_name might be a separate column
      }
      setIsLoading(false);
    }
    loadProfile();
  ...
  const handleSave = async () => {
    ...
    // Update metadata and profiles
    const { error: profileError } = await supabase
      .from('user_profiles')
      .update({ 
        full_name: businessName.trim(),
        // Add more fields if they exist in the schema
      })
      .eq('id', user.id);
