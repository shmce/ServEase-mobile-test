...
const BookingDetailsScreen = () => {
    ...
    async function fetchBookingDetails() {
      ...
      if (params.id) {
        ...
        const { data, error } = await supabase
          .from('bookings')
          .select('*, provider:provider_profiles(user_profiles(full_name, avatar_url))')
          .eq('id', params.id)
          .single();

        if (data && !error) {
          const displayStatus = data.status.charAt(0).toUpperCase() + data.status.slice(1).replace(/_/g, ' ');
          const scheduledDate = new Date(data.scheduled_at || data.created_at);
          setBooking({
            id: data.id,
            service: data.service_name || 'Service',
            address: data.address || 'Consultation',
            date: scheduledDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
            year: scheduledDate.getFullYear().toString() || '2026',
            time: scheduledDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            status: displayStatus,
            statusRaw: data.status,
            provider: {
              name: data.provider?.user_profiles?.full_name || 'Provider',
              rating: 4.9,
              specialty: data.service_name || 'Service',
              avatar: data.provider?.user_profiles?.avatar_url || 'https://via.placeholder.com/150',
              isVerified: true,
            },
            totalAmount: (data.total_price || 0).toLocaleString(),
            countdown: {
              days: '3',
              hours: '4',
              mins: '19',
          ...
          // Set up Realtime listener for this specific booking
          channel = supabase
            .channel(`booking-updates-${params.id}`)
            .on(
              'postgres_changes',
              {
                event: 'UPDATE',
        ...
        try {
          const parsed = JSON.parse(params.booking);
          setBooking(parsed);
        } catch {}
        setIsLoading(false);
      }
  ...
  }, [params.id, params.booking]);

  const bookingStatus = (booking?.statusRaw || booking?.status || '').toLowerCase().replace(/ /g, '_');
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.scheduleContainer}>
            <View style={styles.scheduleCard}>
              ...
              <Text style={styles.scheduleYear}>{booking.year || '2026'}</Text>
            </View>
            <View style={styles.scheduleCard}>
            ...
            <View style={styles.countdownSection}>
              ...
              <View style={styles.timerRow}>
                <View style={styles.timerItem}>
                  <View style={styles.timerBox}><Text style={styles.timerNumber}>{booking.countdown?.days || '0'}</Text></View>
                  <Text style={styles.timerLabel}>DAYS</Text>
                </View>
                <Text style={styles.timerSeparator}>:</Text>
                <View style={styles.timerItem}>
                  <View style={styles.timerBox}><Text style={styles.timerNumber}>{booking.countdown?.hours || '0'}</Text></View>
                  <Text style={styles.timerLabel}>HRS</Text>
                </View>
                <Text style={styles.timerSeparator}>:</Text>
                <View style={styles.timerItem}>
                  <View style={styles.timerBox}><Text style={styles.timerNumber}>{booking.countdown?.mins || '0'}</Text></View>
                  <Text style={styles.timerLabel}>MINS</Text>
                </View>
              </View>
            </View>
          ) : (
            <View style={[styles.countdownSection, booking?.status === 'Cancelled' && styles.countdownSectionCancelled]}>
              <Text style={styles.summaryTitle}>{booking?.status === 'Completed' ? 'SERVICE COMPLETED' : 'BOOKING CANCELLED'}</Text>
                ...
                <View style={styles.providerNameRow}>
                  ...
                  {booking.provider.isVerified && (
                    <View style={styles.verifiedTag}>
                      ...
                      <Text style={{ fontSize: 10, fontWeight: '700', color: '#00C853' }}>Verified</Text>
                    </View>
                  )}
                </View>
