...
export default function CustomerOnboardingAddressScreen() {
  ...
  const [locationNote, setLocationNote] = useState('Select your location');
  ...
  const handleContinue = async () => {
    if (!isReady || isSubmitting) {
      if (!isReady) Alert.alert('Missing details', 'Please complete the full address before continuing.');
      return;
    }
          ...
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.labelRow}>
              {LABELS.map((item) => (
                ...
                >
                  <Text style={[styles.labelChipText, label === item && styles.labelChipTextActive]}>{item}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <TouchableOpacity style={styles.defaultRow} onPress={() => setIsDefault((value: boolean) => !value)} activeOpacity={0.8}>
