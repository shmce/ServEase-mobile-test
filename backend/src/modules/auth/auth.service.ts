...
export class AuthService {
  ...
  async register(dto: any) {
    try {
      ...
      if (!authData.user) throw new InternalServerErrorException("Auth signup returned no user.");
  ...
  async login(loginDto: LoginUserDto) {
    try {
      ...
      if (!isEmail) {
        const userRecord = await this.userRepository.findByContactNumber(identifier);
        if (!userRecord) throw new UnauthorizedException('Phone number not registered.');
      ...
      const userData = await this.userRepository.findById(userId, 'role, status');
  ...
  async registerProvider(dto: RegisterProviderDto, file: Express.Multer.File) {
    ...
    if (!passwordRegex.test(password)) {
      throw new BadRequestException('Password must be 8-128 chars with uppercase, lowercase, number, and special character.');
    }
    ...
    try {
      await this.userRepository.create({
        id: newUserId, full_name, email, contact_number, role, status: 'pending', is_verified: false, date_of_birth,
      });
    } catch (userError) {
    ...
    const { error: docError } = await this.catalogDb.from('provider_documents').insert([{
      provider_id: newUserId, document_type, document_file_path: filePath, status: 'pending',
    }]);
    ...
    return {
      ...
      data: { provider_id: newUserId, business_name: profile.business_name, verification_status: profile.verification_status },
    };
  }
}
