...
export class PaymentsService {
  ...
  async getProviderEarningsSummary(providerId: string) {
    ...
    const paidPayments = (payments as any[]).filter((p) => String(p.status).toLowerCase() === 'completed');
  ...
  async ensureBookingPayment(input: { bookingId: string; customerId: string; providerId: string; amount: number; method?: string }) {
    const { data: existing } = await this.paymentDb.from('payments').select('id, status, amount').eq('booking_id', input.bookingId).maybeSingle();
  ...
  async markBookingPaymentPaid(input: { bookingId: string; amount?: number; customerId?: string; providerId?: string; method?: string }) {
    ...
    const { data: existing } = await this.paymentDb.from('payments').select('id, method, transaction_reference').eq('booking_id', input.bookingId).maybeSingle();
    ...
    if (!existing) {
      ...
      if (!created.payment) throw new InternalServerErrorException('Failed to ensure payment creation');
      const { data, error } = await this.paymentDb.from('payments').update({ status: 'completed', paid_at: now, transaction_reference: createRef('PAID') }).eq('id', created.payment.id).select('id, status, paid_at').maybeSingle();
    ...
    const { data, error } = await this.paymentDb
      .from('payments')
      .update({ status: 'completed', paid_at: now, method: input.method || existing.method || 'cash', transaction_reference: existing.transaction_reference || createRef('PAID') })
      .eq('id', existing.id)
  ...
  async cancelBookingPayment(bookingId: string) {
    const { data: existing } = await this.paymentDb.from('payments').select('id').eq('booking_id', bookingId).maybeSingle();
    if (!existing) return { payment: null };

    const { data, error } = await this.paymentDb.from('payments').update({ status: 'cancelled' }).eq('id', existing.id).select('id, status').maybeSingle();
  ...
  async updateBookingPaymentAmount(bookingId: string, amount: number) {
    ...
    const { data, error } = await this.paymentDb.from('payments').update({ amount: Number(amount || 0) }).eq('id', existing.id).select('id, amount').maybeSingle();
