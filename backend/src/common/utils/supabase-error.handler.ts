 ...
 */
export function handleSupabaseError(error: PostgrestError, context = 'Database'): never {
  ...
  switch (error.code) {
    case '23505': // unique_violation
      throw new ConflictException(`${context}: Resource already exists. Details: ${error.details}`);
    
    case '23503': // foreign_key_violation
      throw new BadRequestException(`${context}: Related resource not found. Details: ${error.details}`);
    
    case 'PGRST116': // no rows returned
      throw new NotFoundException(`${context}: Not found.`);

    case '23502': // not_null_violation
      throw new BadRequestException(`${context}: Missing required field. Details: ${error.details}`);
